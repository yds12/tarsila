...
impl LayersPanel {
    ...
    pub fn update(&mut self, egui_ctx: &egui::Context) -> Vec<Event<WrappedImage>> {
        ...
        egui::Window::new("Layers").show(egui_ctx, |ui| {
            let btn = ui.button("+");
            if btn.clicked() {
                events.push(Event::NewLayerAbove);
            }
            ...
            for i in 0..self.num_layers {
                ui.horizontal(|ui| {
                    ...
                    if ui.radio(i == self.active_layer, "").clicked() {
                        events.push(Event::SwitchLayer(i));
                    }
                    if ui.radio(self.layers_vis[i], "").clicked() {
                        events.push(Event::ChangeLayerVisibility(i, !self.layers_vis[i]));
                    }
                    ...
                    if text_edit.changed() {
                        if let Ok(opacity) = self.layers_alpha[i].parse() {
                            events.push(Event::ChangeLayerOpacity(i, opacity));
                        }
                    }
                    let btn = ui.button("x");
                    if btn.clicked() {
                        events.push(Event::DeleteLayer(i));
                    }
                });
            }
        });
